...
async def test_idempotency_duplicate_returns_saved_response(db_session):
    ...
    async with db_session.begin():
        result = await reserve_idempotency(db_session, "client_1", "idem-1", request_hash)
        assert result.state == "reserved"

    async with db_session.begin():
        await complete_idempotency(db_session, "client_1", "idem-1", response)

    async with db_session.begin():
        dup = await reserve_idempotency(db_session, "client_1", "idem-1", request_hash)
...
async def test_idempotency_in_progress_raises(db_session):
    ...
    async with db_session.begin():
        result = await reserve_idempotency(db_session, "client_1", "idem-2", request_hash)
        assert result.state == "reserved"

    async with db_session.begin():
        with pytest.raises(IdempotencyInProgress):
            await reserve_idempotency(db_session, "client_1", "idem-2", request_hash)
...
async def test_idempotency_conflict_raises(db_session):
    ...
    async with db_session.begin():
        await reserve_idempotency(db_session, "client_1", "idem-3", request_hash)

    async with db_session.begin():
        await complete_idempotency(db_session, "client_1", "idem-3", response)
    ...
    async with db_session.begin():
        with pytest.raises(IdempotencyConflict):
            await reserve_idempotency(db_session, "client_1", "idem-3", request_hash_2)
